...
app.post('/', (req, res) => {
    ...
    res.send(`Enviado: ${req.body.nome}`) // Aqui define uma rota para o caminho '/' (página inicial) usando o método POST. Quando um cliente fizer uma requisição POST para essa rota, 
    // a função de callback será executada, e a resposta 'Formulário recebido!' será enviada de volta para o cliente.
});


app.get('/sobre/:idUsuarios', (req, res) => {
    console.log(req.params);
    res.send(String(req.params.idUsuarios)) // Aqui define uma rota para o caminho '/sobre' usando o método GET. Quando um cliente fizer uma requisição para essa rota, 
